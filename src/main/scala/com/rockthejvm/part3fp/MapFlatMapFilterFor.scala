...
object MapFlatMapFilterFor {
  ...
  // map
  val aIncrementedList = aList.map(_ + 1)
  ...
  /*
    ...
    lambda(4) = ..
  */
  val combinations = numbers.withFilter(_ % 2 == 0).flatMap(number => // List[List[String] => List[String]
    chars.flatMap(char => // List[List[String]] => List[String]
      colors.map(color => s"$number$char - $color") // List[String]
    )
  )
  ...
  */
  ...
  val toPairLList: Int => LList[Int] = x => Cons(x, Cons(x + 1, Empty()))
