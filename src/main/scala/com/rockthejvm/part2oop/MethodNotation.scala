...
//noinspection NoTargetNameAnnotationForOperatorLikeDefinition,PostfixUnaryOperation,TypeAnnotation,ScalaUnusedSymbol
object MethodNotation {
   ...
   *   - a UNARY + operator that increases the person's age +mary => new
   *     Person(_, _, age + 1)
   *   - an apply method with an int arg mary.apply(2) => "Mary watched
   *     Inception 2 times"
   */
